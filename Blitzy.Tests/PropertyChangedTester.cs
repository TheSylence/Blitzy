...
namespace Blitzy.Tests
{
	...
	internal class PropertyChangedTester
	{
		...
		public void RegisterTypeFactory( Type type, Func<object> factory )
		{
			TypeFactories.Add( type, factory );
		}
		...
		private static object GetNonDefaultValueInternal( Type type )
		{
		...
		{
			Func<object> factory;
			if( TypeFactories.TryGetValue( type, out factory ) )
			{
				return factory();
			}

			return GetNonDefaultValueInternal( type );
		}

		private readonly List<string> Errors = new List<string>();
		private readonly INotifyPropertyChanged Object;
		private readonly Dictionary<Type, Func<object>> TypeFactories = new Dictionary<Type, Func<object>>();
	}
}